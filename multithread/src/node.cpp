...
void 
Node::collect (std::vector<std::string>& results, const std::string& pattern){
	auto res = std::mismatch(pattern.begin(), pattern.end(), this->_prefix.begin());
	// Exit if the branch does not match to the pattern
